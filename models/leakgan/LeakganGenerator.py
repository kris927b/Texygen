...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
        ...
        with tf.compat.v1.variable_scope('place_holder'):
            ...
            self.reward = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size,
            ...
            self.drop_out = tf.compat.v1.placeholder(tf.float32, name="dropout_keep_prob")
            self.train = tf.compat.v1.placeholder(tf.int32, None, name="train")

        with tf.compat.v1.variable_scope('Worker'):
            self.g_embeddings = tf.Variable(tf.random.normal([self.num_vocabulary, self.emb_dim], stddev=0.1))
            ...
            self.W_workerOut_change = tf.Variable(tf.random.normal([self.num_vocabulary, self.goal_size], stddev=0.1))

            self.g_change = tf.Variable(tf.random.normal([self.goal_out_size, self.goal_size], stddev=0.1))
        ...
        with tf.compat.v1.variable_scope('Manager'):
            ...
            self.goal_init = tf.compat.v1.get_variable("goal_init",
                                             initializer=tf.random.truncated_normal([self.batch_size, self.goal_out_size],
                                                                             stddev=0.1))
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            cur_sen = tf.cond(pred=i > 0, true_fn=lambda:
            tf.split(tf.concat([tf.transpose(a=gen_x.stack(), perm=[1, 0]), self.padding_array], 1),
                     [self.sequence_length, i], 1)[0], false_fn=lambda: self.padding_array)
            ...
            log_prob = tf.math.log(tf.nn.softmax(
                tf.cond(pred=i > 1, true_fn=lambda: tf.cond(pred=self.train > 0, true_fn=lambda: self.tem, false_fn=lambda: 1.5), false_fn=lambda: 1.5) * x_logits))
            ...
            gen_o = gen_o.write(i, tf.reduce_sum(input_tensor=tf.multiply(tf.one_hot(next_token, self.num_vocabulary, 1.0, 0.0),
                                                             tf.nn.softmax(x_logits)), axis=1))  # [batch_size] , prob
            return i + 1, x_tp1, h_t_Worker, h_t_manager, gen_o, gen_x, goal, \
                   tf.cond(pred=((i + 1) % step_size) > 0, true_fn=lambda: real_sub_goal,
                           false_fn=lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])) \
                , tf.cond(pred=((i + 1) % step_size) > 0, true_fn=lambda: real_goal,
                          false_fn=lambda: real_sub_goal), step_size, gen_real_goal_array, gen_o_worker_array
        ...
        _, _, _, _, self.gen_o, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.start_token), self.h0_worker, self.h0_manager,
        ...
        self.gen_real_goal_array = tf.transpose(a=self.gen_real_goal_array,
        ...
        self.gen_o_worker_array = tf.transpose(a=self.gen_o_worker_array,
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            real_goal_array = tf.cond(pred=i > 0, true_fn=lambda: real_goal_array,
                                      false_fn=lambda: real_goal_array.write(0, self.goal_init))
            h_t_manager = self.g_manager_recurrent_unit(feature, h_tm1_manager)
            sub_goal = self.g_manager_output_unit(h_t_manager)
            sub_goal = tf.nn.l2_normalize(sub_goal, 1)

            h_t_Worker = tf.cond(pred=i > 0, true_fn=lambda: self.g_worker_recurrent_unit(x_t, h_tm1),
                                 false_fn=lambda: h_tm1)  # hidden_memory_tuple
            o_t_Worker = self.g_worker_output_unit(h_t_Worker)  # batch x vocab , logits not prob
            o_t_Worker = tf.reshape(o_t_Worker, [self.batch_size, self.num_vocabulary, self.goal_size])

            real_sub_goal = tf.cond(pred=i > 0, true_fn=lambda: tf.add(last_goal, sub_goal),
                                    false_fn=lambda: real_goal)
            all_sub_goals = tf.cond(pred=i > 0, true_fn=lambda: all_sub_goals.write(i - 1, real_goal),
                                    false_fn=lambda: all_sub_goals)
            ...
            g_predictions = tf.cond(pred=i > 0, true_fn=lambda: g_predictions.write(i - 1, tf.nn.softmax(x_logits)),
                                    false_fn=lambda: g_predictions)

            sub_feature = tf.cond(pred=((((i) % step_size) > 0)),
                                  true_fn=lambda: sub_feature,
                                  false_fn=lambda: (tf.cond(pred=i > 0, true_fn=lambda: sub_feature.write(i // step_size - 1,
                                                                                    tf.subtract(feature,
                                                                                                feature_array.read(
                                                                                                    i - step_size))),
                                                   false_fn=lambda: sub_feature)))

            all_sub_features = tf.cond(pred=i > 0, true_fn=lambda: tf.cond(pred=(i % step_size) > 0, true_fn=lambda: all_sub_features.write(i - 1,
                                                                                                                  tf.subtract(
                                                                                                                      feature,
                                                                                                                      feature_array.read(
                                                                                                                          i - i % step_size))), \
                                                              false_fn=lambda: all_sub_features.write(i - 1, tf.subtract(feature,
                                                                                                                feature_array.read(
                                                                                                                    i - step_size)))),
                                       false_fn=lambda: all_sub_features)

            real_goal_array = tf.cond(pred=((i) % step_size) > 0, true_fn=lambda: real_goal_array,
                                      false_fn=lambda: tf.cond(pred=(i) / step_size < self.sequence_length // step_size,
                                                      true_fn=lambda: tf.cond(pred=i > 0,
                                                                      true_fn=lambda: real_goal_array.write((i) // step_size,
                                                                                                    real_sub_goal),
                                                                      false_fn=lambda: real_goal_array),
                                                      false_fn=lambda: real_goal_array))
            x_tp1 = tf.cond(pred=i > 0, true_fn=lambda: ta_emb_x.read(i - 1),
                            false_fn=lambda: x_t)

            return i + 1, x_tp1, g_predictions, h_t_Worker, input_x, h_t_manager, \
                   tf.cond(pred=((i) % step_size) > 0, true_fn=lambda: real_sub_goal,
                           false_fn=lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
                   tf.cond(pred=((i) % step_size) > 0, true_fn=lambda: real_goal, false_fn=lambda: real_sub_goal), \
        ...
        _, _, self.g_predictions, _, _, _, _, _, self.feature_array, self.real_goal_array, self.sub_feature, self.all_sub_features, self.all_sub_goals = control_flow_ops.while_loop(
            ...
            body=preTrain,
            loop_vars=(
            tf.constant(0, dtype=tf.int32), tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.start_token), g_predictions,
            self.h0_worker,
        ...
        self.sub_feature = tf.transpose(a=self.sub_feature, perm=[1, 0, 2])

        self.real_goal_array = self.real_goal_array.stack()
        self.real_goal_array = tf.transpose(a=self.real_goal_array, perm=[1, 0, 2])
        print(self.real_goal_array.shape)
        print(self.sub_feature.shape)
        self.pretrain_goal_loss = -tf.reduce_sum(input_tensor=1 - tf.compat.v1.losses.cosine_distance(tf.nn.l2_normalize(self.sub_feature, 2),
        ...
        with tf.compat.v1.name_scope("Manager_PreTrain_update"):
            ...
            self.pretrain_manager_grad, _ = tf.clip_by_global_norm(
                tf.gradients(ys=self.pretrain_goal_loss, xs=self.manager_params), self.grad_clip)
            self.pretrain_manager_updates = pretrain_manager_opt.apply_gradients(
                zip(self.pretrain_manager_grad, self.manager_params))
        # self.real_goal_array = self.real_goal_array.stack()

        self.g_predictions = tf.transpose(a=self.g_predictions.stack(),
                                          perm=[1, 0, 2])  # batch_size x seq_length x vocab_size
        self.cross_entropy = tf.reduce_sum(
            input_tensor=self.g_predictions * tf.math.log(tf.clip_by_value(self.g_predictions, 1e-20, 1.0))) / (
        ...
        with tf.compat.v1.name_scope("Worker_PreTrain_update"):
            ...
            self.pretrain_worker_grad, _ = tf.clip_by_global_norm(
                tf.gradients(ys=self.pretrain_worker_loss, xs=self.worker_params), self.grad_clip)
            self.pretrain_worker_updates = pretrain_worker_opt.apply_gradients(
                zip(self.pretrain_worker_grad, self.worker_params))

        self.goal_loss = -tf.reduce_sum(input_tensor=tf.multiply(self.reward, 1 - tf.compat.v1.losses.cosine_distance(
        ...
        with tf.compat.v1.name_scope("Manager_update"):
            ...
            self.manager_grad, _ = tf.clip_by_global_norm(
                tf.gradients(ys=self.goal_loss, xs=self.manager_params), self.grad_clip)
        ...
        self.all_sub_features = tf.transpose(a=self.all_sub_features, perm=[1, 0, 2])

        self.all_sub_goals = self.all_sub_goals.stack()
        self.all_sub_goals = tf.transpose(a=self.all_sub_goals, perm=[1, 0, 2])
        # self.all_sub_features = tf.nn.l2_normalize(self.all_sub_features, 2)
        self.Worker_Reward = 1 - tf.compat.v1.losses.cosine_distance(tf.nn.l2_normalize(self.all_sub_features, 2),
                                                           tf.nn.l2_normalize(self.all_sub_goals, 2), 2)
        # print self.Worker_Reward.shape
        self.worker_loss = -tf.reduce_sum(
            input_tensor=tf.multiply(self.Worker_Reward,
                        tf.one_hot(tf.cast(tf.reshape(self.x, [-1]), dtype=tf.int32), self.num_vocabulary, 1.0, 0.0) * tf.math.log(
                            tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_vocabulary]), 1e-20, 1.0))
                        )
        ) / (self.sequence_length * self.batch_size)
        with tf.compat.v1.name_scope("Worker_update"):
            ...
            self.worker_grad, _ = tf.clip_by_global_norm(
                tf.gradients(ys=self.worker_loss, xs=self.worker_params), self.grad_clip)
    ...
    def rollout(self, input_x, given_num):
        with tf.device("/cpu:0"):
            processed_x = tf.transpose(a=tf.nn.embedding_lookup(params=self.g_embeddings, ids=input_x),
                                       perm=[1, 0, 2])  # seq_length x batch_size x emb_dim
        ...
        ta_x = ta_x.unstack(tf.transpose(a=input_x, perm=[1, 0]))
        ...
        def _g_recurrence_1(i, x_t, input_x, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal, give_num):
            ...
            real_sub_goal = tf.cond(pred=i > 0, true_fn=lambda: tf.add(last_goal, sub_goal), false_fn=lambda: real_goal)
            # real_goal_array = real_goal_array.write(i, real_sub_goal)

            x_tp1 = tf.cond(pred=i > 0, true_fn=lambda: ta_emb_x.read(i - 1), false_fn=lambda: x_t)

            # hidden_memory_tuple
            with tf.control_dependencies([cur_sen]):
                gen_x = tf.cond(pred=i > 0, true_fn=lambda: gen_x.write(i - 1, ta_x.read(i - 1)), false_fn=lambda: gen_x)
            return i + 1, x_tp1, input_x, gen_x, h_t_Worker, h_t_manager, \
                   tf.cond(pred=((i) % self.step_size) > 0, true_fn=lambda: real_sub_goal,
                           false_fn=lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
                   tf.cond(pred=((i) % self.step_size) > 0, true_fn=lambda: real_goal, false_fn=lambda: real_sub_goal), give_num
        ...
        def _g_recurrence_2(i, x_t, gen_x, h_tm1, h_tm1_manager, last_goal, real_goal):
            ...
            cur_sen = tf.cond(pred=i > 0, true_fn=lambda:
            tf.split(tf.concat([tf.transpose(a=gen_x.stack(), perm=[1, 0]), self.padding_array], 1),
                     [self.sequence_length, i - 1], 1)[0], false_fn=lambda: self.padding_array)
            with tf.compat.v1.variable_scope(self.scope):
                feature = self.FeatureExtractor_unit(cur_sen, self.drop_out)
            h_t_Worker = self.g_worker_recurrent_unit(x_t, h_tm1)  # hidden_memory_tuple
            o_t_Worker = self.g_worker_output_unit(h_t_Worker)  # batch x vocab , logits not prob
            ...
            log_prob = tf.math.log(tf.nn.softmax(x_logits))
            next_token = tf.cast(tf.reshape(tf.random.categorical(logits=log_prob, num_samples=1), [self.batch_size]), tf.int32)
            x_tp1 = tf.nn.embedding_lookup(params=self.g_embeddings, ids=next_token)  # batch x emb_dim
            with tf.control_dependencies([cur_sen]):
            ...
            return i + 1, x_tp1, gen_x, h_t_Worker, h_t_manager, \
                   tf.cond(pred=((i) % self.step_size) > 0, true_fn=lambda: real_sub_goal,
                           false_fn=lambda: tf.constant(0.0, shape=[self.batch_size, self.goal_out_size])), \
                   tf.cond(pred=((i) % self.step_size) > 0, true_fn=lambda: real_goal, false_fn=lambda: real_sub_goal)
        ...
        i, x_t, _, gen_for_reward, h_worker, h_manager, self.last_goal_for_reward, self.real_goal_for_reward, given_num = control_flow_ops.while_loop(
            ...
            loop_vars=(
            tf.constant(0, dtype=tf.int32), tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.start_token), self.x,
            gen_for_reward,
        ...
        gen_for_reward = tf.transpose(a=gen_for_reward, perm=[1, 0])  # batch_size x seq_length
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.compat.v1.variable_scope('Worker'):
            ...
            self.Wi_worker = tf.Variable(tf.random.normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Ui_worker = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi_worker = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wf_worker = tf.Variable(tf.random.normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uf_worker = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bf_worker = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wog_worker = tf.Variable(tf.random.normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uog_worker = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bog_worker = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wc_worker = tf.Variable(tf.random.normal([self.emb_dim, self.hidden_dim], stddev=0.1))
            self.Uc_worker = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bc_worker = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))
            params.extend([
    ...
    def create_Worker_output_unit(self, params):
        with tf.compat.v1.variable_scope('Worker'):
            self.W_worker = tf.Variable(
                tf.random.normal([self.hidden_dim, self.num_vocabulary * self.goal_size], stddev=0.1))
            self.b_worker = tf.Variable(tf.random.normal([self.num_vocabulary * self.goal_size], stddev=0.1))
    ...
    def create_Manager_recurrent_unit(self, params):
        with tf.compat.v1.variable_scope('Manager'):
            # Weights and Bias for input and hidden tensor
            self.Wi = tf.Variable(tf.random.normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Ui = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bi = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wf = tf.Variable(tf.random.normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uf = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bf = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wog = tf.Variable(tf.random.normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uog = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bog = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))

            self.Wc = tf.Variable(tf.random.normal([self.num_filters_total, self.hidden_dim], stddev=0.1))
            self.Uc = tf.Variable(tf.random.normal([self.hidden_dim, self.hidden_dim], stddev=0.1))
            self.bc = tf.Variable(tf.random.normal([self.hidden_dim], stddev=0.1))
            params.extend([
    ...
    def create_Manager_output_unit(self, params):
        with tf.compat.v1.variable_scope('Manager'):
            self.Wo = tf.Variable(tf.random.normal([self.hidden_dim, self.goal_out_size], stddev=0.1))
            self.bo = tf.Variable(tf.random.normal([self.goal_out_size], stddev=0.1))
