...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim,
        ...
        self.y = tf.compat.v1.placeholder(tf.int32, shape=[self.batch_size,
        ...
        # Initial states
        self.h_0 = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, emb_dim])
        self.c_0 = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, emb_dim])
        ...
        _, _, _, self.gen_o, self.gen_x, self.gen_ot = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.start_token), self.h0, gen_o, gen_x, gen_ot))
        ...
        self.gen_ot = tf.transpose(a=self.gen_ot, perm=[1, 0, 2])  # batch_size x seq_length x g_emb_dim
        ...
        self.g_grad, _ = tf.clip_by_global_norm(tf.gradients(ys=self.mmd, xs=self.g_params), self.grad_clip)
    ...
    def init_matrix(self, shape):
        return tf.random.normal(shape, stddev=5)
