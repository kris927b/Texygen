...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, hidden_dim,
        ...
        with tf.device("/cpu:0"):
            self.processed_x = tf.transpose(a=tf.nn.embedding_lookup(params=onehot, ids=self.x),
                                            perm=[1, 0, 2])  # seq_length x batch_size x emb_dim

        # Initial states
        self.h_0 = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, hidden_dim])
        self.c_0 = tf.compat.v1.placeholder(tf.float32, shape=[batch_size, hidden_dim])
        ...
        _, _, h_t, self.gen_o, self.gen_x, self.gen_ot = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=onehot, ids=self.start_token), self.h0, gen_o, gen_x, gen_ot))

        self.gen_x = self.gen_x.stack()  # seq_length x batch_size
        self.gen_x = tf.transpose(a=self.gen_x, perm=[1, 0])  # batch_size x seq_length

        self.gen_ot = self.gen_ot.stack()
        self.gen_ot = tf.transpose(a=self.gen_ot, perm=[1, 0, 2]) #  batch_size x seq_length x num_vocabulary
        ...
        _, _, _, self.g_predictions = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=onehot, ids=self.start_token),
        ...
        g_grad, _ = tf.clip_by_global_norm(tf.gradients(ys=self.g_loss, xs=self.g_params), self.grad_clip)
    ...
    def add_gumbel(self, hidden_memory_tuple):
        ...
        hidden_state = tf.add(hidden_state, self.get_gumbel(tf.shape(input=hidden_state)))
    ...
    def set_similarity(self, valid_examples=None, pca=True):
        ...
        self.norm = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(one_hot), axis=1, keepdims=True))
